  ...
  <div class="max-w-[1200px] w-[95%] mx-auto mt-[-100px] relative z-100">
    ...
    >
      <div>
        ...
        <div
          class="px-2 pb-2 text-white flex flex-col items-start md:w-[632px] md:mx-auto md:px-0 lg:flex-row lg:items-center lg:w-[732px]"
        >
        ...
        >
          ...
          <div class="lg:w-[350px] md:w-[300px]">
            <EcoSelect
              :label="t('forward.importingEconomy')"
...
// 📌 Select type: Single Year / Time Range
const selectType = ref("Importing Economy");
  ...
  if (checkYearPass === 1) {
    alert(
      `${t('forward.limitedData')}\n\n` +
        `${t('backward.availability', {
          exporting: translateEconomy(inputData.value.exporting, locale.value),
          importing: translateEconomy(inputData.value.importing, locale.value),
...
watch(
  ...
  async ([exp, imp, yearStart, yearEnd]) => {
    if (!exp || !imp || yearStart || yearEnd) return;
    ...
    try {
      ...
      console.error("Error loading available forward period:", error);
    } finally {
