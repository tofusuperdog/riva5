      ...
      <div>
        ...
        >
          <div class="lg:w-[350px] md:w-[300px]">
            <EcoSelect
              :label="t('backward.exportingEconomy')"
          ...
          <div class="lg:w-[350px] md:w-[300px]">
            <EcoSelect
              :label="t('backward.importingEconomy')"
        ...
        >
          <div class="lg:w-[350px] md:w-[300px]">
            <EcoSelect
              :label="t('backward.sourceEconomy')"
              :initialValue="sourceISOInit"
              @update:selected="onUpdateSourceISO"
            />
          </div>
        ...
        <div class="h-6 text-center text-yellow-300">
          ...
          <span v-show="showInvalidYear">{{ t("backward.yearRangeError") }}</span>
        </div>
      </div>
    </div>
  </div>
...
// 📌 Select type: Single Year / Time Range
const selectType = ref("Source Economy");
...
// 🌍 Source Economy updated
const onUpdateSourceISO = (selected) => {
  emit("isShowGraph", false);
  isInputApply.value = false;
  inputData.value.source = selected;
  updateLocalStorage("sourceISO", selected.iso);
  updateLocalStorage("sourceName", selected.name);
  if (
    inputData.value.exporting &&
    inputData.value.importing &&
    inputData.value.yearStart &&
...
const onClickApply = async () => {
  ...
  // check for source
  const payLoad3 = {
    exp_country: inputData.value.source.iso,
  };
  ...
  if (checkYearPass === 1) {
    alert(
      `${t("backward.limitedData")}\n\n` +
        `${t("backward.availabilityWithSource", { exporting: inputData.value.exporting.name, importing: inputData.value.importing.name, source: inputData.value.source.name, start: yearRangeShow[0], end: yearRangeShow[1] })}\n` +
        t("backward.adjustPeriod")
    );
    return;
  }

  Notify.create({
    message: t("backward.applied"),
    color: "positive",
    position: "center",
    timeout: 2000,
...
// 🧭 Watch for route param
// 🧠 Sync params.source → sourceISOInit
watch(
  [() => route.params.source, () => economyList.value],
  ([source, ecoList]) => {
    if (ecoList.length === 0) return;
    ...
    if (!isoToUse) {
      const stored = LocalStorage.getItem("inputVA");
      if (stored && stored.sourceISO) {
        isoToUse = stored.sourceISO;
      }
    }
    ...
    if (matched) {
      ...
      sourceISOInit.value = sourceDataInput;
      onUpdateSourceISO(sourceDataInput);
      updateLocalStorage("sourceISO", matched.value);
      updateLocalStorage("sourceName", matched.label);
    }
  },
  { immediate: true }
);
