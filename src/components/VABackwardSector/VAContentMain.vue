      ...
      <div>
        ...
        <div class="h-6 text-center text-yellow-300">
          ...
          <span v-show="showInvalidYear">
            {{ t("backward.yearRangeError") }}</span
          >
        </div>
      </div>
    </div>
...
const getLocalizedInputData = () => {
  ...
  const importing = inputData.value.importing;
  const sector = inputData.value.sector;
  const localizedSectorName = sector
    ? translateSector(
        { catID: sector.sectorID, category: sector.sectorName },
        locale.value,
      )
    : "";
  ...
  return {
    ...
    exporting: exporting
      ? {
          ...exporting,
          name: translateEconomy(exporting, locale.value),
        }
      : exporting,
    importing: importing
      ? {
          ...importing,
          name: translateEconomy(importing, locale.value),
        }
      : importing,
    sector: sector
      ? {
          ...sector,
          sectorName: localizedSectorName,
          sectorShortName: localizedSectorName,
        }
      : sector,
  };
};
...
const onChangeRoute = () => {
  if (selectType.value === "Source Economy") {
  ...
  if (checkYearPass === 1) {
    alert(
      `${t("backward.limitedData")}\n\n` +
        `${t("backward.availability", { exporting: inputData.value.exporting.name, importing: inputData.value.importing.name, start: yearRangeShow[0], end: yearRangeShow[1] })}\n` +
...
const loadEconomyList = async () => {
  try {
    ...
    economyList.value = res.data.map((data) => ({
      id: data.id,
      label: translateEconomy(data, locale.value, data.name === "Economy Groups" ? 1 : 0),
      value: data.iso,
...
const loadSectorList = async () => {
  try {
    ...
    sectorList.value = res.data.map((data) => ({
      sectorName: translateSector(data, locale.value),
      sectorID: data.catID,
      sectorShortName: translateSector(data, locale.value),
